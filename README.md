...
| `socials.json` | Ссылки на соцсети автора в шапке сборки — цветными кнопками |
| `banner.png` | Баннер сборки в шапке вкладки «Релизы» |
...
Необязательный `banner.png` в корне репозитория (не в релизе!) — лаунчер показывает его
в шапке сборки. Рекомендуемые размеры — 1200×360, PNG/JPG, до ~1 МБ.
